# Create a raw transaction that can be spent in 2 weeks time, assuming the current block is 25

# Amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
# transaction=""

#!/bin/bash
bitcoin-cli -named createrawtransaction inputs='[{"txid":"c8b0928edebbec5e698d5f86d0474595d9f6a5b2e4e3772cd9d1005f23bdef77250000000000000000000000000000000000000000000000000000000₀₀₀₀₀','vout':０}]' outputs='{"2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP":０.２}' locktime=２７
